...
/**
 * Returns a "Lire la suite" link for excerpts
 */
function athemes_continue_reading_link() {
	return ' <a href="'. esc_url( get_permalink() ) . '" class="more-link">' . __( 'Lire la suite', 'lepays' ) . ' <span class="meta-nav">&rarr;</span></a>';
}

/**
 * Adds a pretty "Lire la suite" link to custom post excerpts.
 *
...
function athemes_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'lepays' ), max( $paged, $page ) );
